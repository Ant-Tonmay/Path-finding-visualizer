...
import { eraseWall } from "../../../utils/eraseWall";
    ...
    for(let row =1 ; row<MAX_ROWS ; row+=2){
        for(let col =1 ; col<MAX_COLS ; col+=2){
            if(row === MAX_ROWS -2 && col === MAX_COLS -2){
                ...
                await eraseWall(grid,row,col,1,speed)
            }else if(col===MAX_COLS -2){
                await eraseWall(grid,row,col,0,speed)
            }else{
                await eraseWall(grid,row,col,getRandInt(0,2),speed)
            }
        }
    }
    setIsDisabled(false);
